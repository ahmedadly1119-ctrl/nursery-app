...
def load_data():
    try:
        ...
        with BytesIO(response.content) as f:
            ...
            ledger = pd.read_excel(f, sheet_name='Ledger')
        ...
        # هيكل الجداول في حالة أول مرة تشغيل
        s = pd.DataFrame(columns=['ID', 'الاسم/Name', 'ولي الأمر/Parent', 'الهاتف/Phone', 'المبلغ الثابت/Fee', 'الحالة/Status', 'تاريخ التسجيل/Reg Date', 'الرصيد/Balance'])
        l = pd.DataFrame(columns=['EntryID', 'التاريخ/Date', 'اسم الطالب/Student', 'النوع/Type', 'البيان/Desc', 'مدين/Debit (+)', 'دائن/Credit (-)', 'الرصيد بعد الحركة/Balance'])
        return s, l

def save_to_cloud(students, ledger):
    try:
        requests.post(WEB_APP_URL, params={"sheet": "Students"}, data=students.to_json(orient='records', date_format='iso'))
        requests.post(WEB_APP_URL, params={"sheet": "Ledger"}, data=ledger.to_json(orient='records', date_format='iso'))
    except: pass

# --- 2. إعدادات الصفحة واللغة (نفس هيكلك الأصلي) ---
...
def record_transaction(name, t_type, desc, amount, custom_date=None):
    ...
    save_to_cloud(st.session_state.students, st.session_state.ledger)

# --- 4. القائمة والواجهات (نفس الهيكل بالحرف) ---
    ...
    with t1:
        with st.form("add_form"):
            ...
            if st.form_submit_button(T['save']):
                ...
                save_to_cloud(st.session_state.students, st.session_state.ledger); st.success("Saved!")
    with t2:
        ...
        with st.form("edit_form"):
            col1, col2 = st.columns(2)
            u_n = col1.text_input(T['st_name'], curr['الاسم/Name']); u_p = col2.text_input(T['parent'], curr['ولي الأمر/Parent'])
            u_ph = col1.text_input(T['phone'], curr['الهاتف/Phone']); u_f = col2.number_input(T['fee'], value=int(curr['المبلغ الثابت/Fee']))
            ...
            if st.form_submit_button(T['update']):
                st.session_state.students.at[idx, 'الاسم/Name'] = u_n; st.session_state.students.at[idx, 'ولي الأمر/Parent'] = u_p
                st.session_state.students.at[idx, 'الهاتف/Phone'] = u_ph; st.session_state.students.at[idx, 'المبلغ الثابت/Fee'] = u_f
                st.session_state.students.at[idx, 'الرصيد/Balance'] = u_bal; st.session_state.students.at[idx, 'الحالة/Status'] = u_st
                st.session_state.students.at[idx, 'تاريخ التسجيل/Reg Date'] = u_rd
                save_to_cloud(st.session_state.students, st.session_state.ledger); st.success("Updated!")

# 4.3 الفوترة والتسويات (تلاتة تابات بنفس الترتيب)
elif menu == T['menu'][2]:
        ...
        if st.button(T['confirm'], key="bulk"):
            ...
            for _, s in active.iterrows():
                if s['تاريخ التسجيل/Reg Date'] <= last_d:
                    ...
                    if not exists: record_transaction(s['الاسم/Name'], "Invoice", desc, s['المبلغ الثابت/Fee']); done += 1
    ...
    with t2:
        with st.form("extra_form"):
            ...
            h = col_a.number_input("Hours", min_value=0.5, step=0.5); pr = col_b.number_input("Rate", min_value=0.0)
            ex_date = st.date_input("Service Date", datetime.date.today())
            note = st.text_input("Notes")
            if st.form_submit_button(T['confirm']):
                record_transaction(stb, "Invoice", f"ساعات إضافية ({h} س) - {note}", h*pr, ex_date); st.success("Added!")
    with t3:
        with st.form("adj_form"):
            ...
            if st.form_submit_button(T['confirm']):
                record_transaction(stb, type_adj, reason, amt); st.success("Done!")
...
# 4.4 التحصيل
    ...
    with st.form("pay_form"):
        ...
        dt = st.date_input("Date", datetime.date.today()); note = st.text_input("Notes")
        if st.form_submit_button(T['confirm']):
            record_transaction(stb, "Payment", note, amt, dt); st.success("Paid!")
...
# 4.5 التقارير
    ...
    with t_rep1:
        ...
        st.download_button(T['download'], to_excel(st.session_state.students), "Balances.xlsx")
    with t_rep2:
        ...
        if st.button(T['delete_last']):
            if not res.empty:
                last_idx = res.index[-1]; s_idx = st.session_state.students[st.session_state.students['الاسم/Name'] == stb].index[0]
                ...
                save_to_cloud(st.session_state.students, st.session_state.ledger); st.rerun()
